    ...
    @endif
      <div class="parent">
          ...
          <div class="fileLeft">     
            ...
            @csrf
              <input name="searchbar" id="searchbar" type="text" placeholder="Nom d'un fichier...">
                    ...
                    @foreach ($file as $f)
                      @if ($f->ownerid == $_SESSION['sessionID'])     
                      <a href="/file/{{$f->uuid}}"><button id="files">{{$f->name}}.c</button></a>
                      @endif
                    @endforeach
          </div>
          <div class="fileOpen">
        ...
        <div class="right">
            ...
            <form method="post" action="/save/{{$fileuuid}}">
              @csrf
              <textarea id="editor" name="editor" class="text">{{$filecontent}}</textarea>
